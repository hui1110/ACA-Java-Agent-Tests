...
export QUARKUS_LOGBACK_RESOURCE_GROUP="quarkuslogbackit"
...
if [ $(az containerapp logs show --name app-$PROJECT_NAME_JAVA_11_ACA --resource-group $QUARKUS_LOGBACK_RESOURCE_GROUP --type console --tail 100 | grep "quarkus-logback-sample" | wc -l) -gt 0 ]
then
...
if [ $(az containerapp logs show --name app-$PROJECT_NAME_JAVA_17_ACA --resource-group $QUARKUS_LOGBACK_RESOURCE_GROUP --type console --tail 100 | grep "quarkus-logback-sample" | wc -l) -gt 0 ]
then
...
if [ $(az containerapp logs show --name app-$PROJECT_NAME_JAVA_21_ACA --resource-group $QUARKUS_LOGBACK_RESOURCE_GROUP --type console --tail 100 | grep "quarkus-logback-sample" | wc -l) -gt 0 ]
then
